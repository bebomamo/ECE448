...
class Application:
    ...
    def execute(self):
        if self.args.early_check:
            ...
            if not np.array_equal(test_results, gold_results):
                print("Warning: Your discretized states do not match gold ones")
                mismatches = np.where(test_results != gold_results)
                for idx in mismatches[0]:
                    print(f"Mismatch at index {idx}: test_result = {test_results[idx]}, gold_result = {gold_results[idx]}")
                match = False
            else:
                print('State discretization tests passed!')
