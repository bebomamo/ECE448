...
class NeuralNet(nn.Module):
    def __init__(self, lrate, loss_fn, in_size, out_size):
        ...
        self.c1 = nn.Conv2d(3, 12, 3)
        self.p1 = nn.MaxPool2d(2,2)
        self.c2 = nn.Conv2d(12, 32, 3)
        self.l1 = nn.Linear(in_features=1152, out_features=256, bias=True) #linear layer 1
        self.l2 = nn.Linear(in_features=256, out_features=128, bias=True) #linear layer 2
        self.out = nn.Linear(in_features=128, out_features=4, bias=False) #output layer
...
def fit(train_set,train_labels,dev_set,epochs,batch_size=100):
    ...
    """
    net = NeuralNet(0.0001, nn.CrossEntropyLoss(), len(train_labels), len(dev_set))
    ...
    # modified_batch_features = remake_features(train_set)
    ...
    # modified_dev_features = remake_features(dev_set)
    outputs = net(dev_set)
...
# def remake_features(features):
#     new_features = np.zeros((len(features),3,31,31)) #list of pictures -> list of RGB selection -> 2D array of pixel locations
#     for i in range(len(features)): #iterating over batch 
#         print("\rCurrent Image # being modded: {}".format(i), end="")
#         for color in range(3): #iterating of R, G, B index
#             for y in range(31):
#                 for x in range(31):
#                     new_features[i][color][y][x] = features[i][y*31*3 + x*3 + color]
#     new_features = np.float32(new_features)
#     print(" -> modification of features complete...")
